...
RegisterServerEvent('md-scrapyard:server:removetools', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local chance = Config.ToolsRemovalChance
	if chance <= math.random(1,100) then
	Player.Functions.RemoveItem(Config.ToolItem, 1) 
	TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items[Config.ToolItem], "remove", 1)
	TriggerClientEvent('QBCore:Notify', src, ("Your tools broke"), "error")
	end
end)
